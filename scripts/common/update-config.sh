...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config config_block.pb -o ${ORDERER}.${EXAMPLE_DOMAIN}:7050 --ordererTLSHostnameOverride ${ORDERER}.${EXAMPLE_DOMAIN} -c $CHANNEL --tls --cafile "$ORDERER_CA"
  { set +x; } 2>/dev/null
