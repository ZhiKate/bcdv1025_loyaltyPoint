...
createAnchorPeerUpdate() {
  ...
  if [ $ORG -eq 1 ]; then
    HOST="peer0.${PEER_ORG1}.${EXAMPLE_DOMAIN}"
    PORT=7051
  elif [ $ORG -eq 2 ]; then
    HOST="peer0.${PEER_ORG2}.${EXAMPLE_DOMAIN}"
    PORT=9051
  elif [ $ORG -eq 3 ]; then
    HOST="peer0.${PEER_ORG3}.${EXAMPLE_DOMAIN}"
    PORT=11051
  else
...
updateAnchorPeer() {
  set -x
  peer channel update -o ${ORDERER}.${EXAMPLE_DOMAIN}:7050 \
